...
# Load trained shuffle model
shuffle_model_path = "/hps/nobackup/goldman/charwest/omega_ai/data/saved_models/2023-08-07-12-27-22_shuffle_divbase_indistNB_indrate01_tips8_posprop05_average_0.5_512_0.001/50"
shuffle_model = tf.keras.models.load_model(shuffle_model_path)
x = tf.ones((512, args.first_height, 2000, 5))
shuffle_model.evaluate(x)
...
for i, aligner in enumerate(aligner_list):
    ...
    for batch in test_dataset:
        ...
        shuffle_predictions = shuffle_model.predict(x_test)
        y_true.extend(y_test.numpy())
        y_pred.extend(predictions.flatten())
        y_shuffle_pred.extend(shuffle_predictions.flatten())
    ...
    # Plot ROC curve
    plt.plot(fpr, tpr, lw=2, color=aligner_colors[aligner], label = f'OmegaAI {aligner_labels[aligner]} (AUC = {roc_auc:.2f})', alpha=0.5) #, label=f'Dataset {i+1} ROC (AUC = {roc_auc:.2f})')

    # SHUFFLE MODEL
     # Compute ROC curve
    fpr, tpr, thresholds = roc_curve(y_true, y_shuffle_pred)
    roc_auc = auc(fpr, tpr)
    # print("these are the thresholds: {0}".format(thresholds))
    print("this is shuffle roc auc for {0}: {1}".format(aligner, roc_auc))
    # Plot ROC curve
    plt.plot(fpr, tpr, lw=2, color=aligner_colors[aligner], label = f'OmegaAI shuffled {aligner_labels[aligner]} (AUC = {roc_auc:.2f})')   
...
for i, aligner in enumerate(aligner_list):
    ...
    plt.plot(recall, precision, lw=2, color=aligner_colors[aligner], label = f'OmegaAI {aligner_labels[aligner]} (AUC = {pr_auc:.2f})', alpha=0.5 ) #, label=f'Dataset {i+1} Precision-Recall (AUC = {pr_auc:.2f})')

    # SHUFFLE MODEL
    # Compute Precision-Recall curve
    precision, recall, _ = precision_recall_curve(y_true, y_shuffle_pred)
    pr_auc = auc(recall, precision)
    # print("these are the thresholds: {0}".format(thresholds))
    print("this is pr auc for {0}: {1}".format(aligner, pr_auc))

    # Plot Precision-Recall curve
    plt.plot(recall, precision, lw=2, color=aligner_colors[aligner], label = f'OmegaAI shuffled {aligner_labels[aligner]} (AUC = {pr_auc:.2f})' )
...
plt.savefig("/hps/nobackup/goldman/charwest/omega_ai/data/benchmark/cnn_shuffle_compare_all_roc_pr.png")
