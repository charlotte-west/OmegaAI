...
for i, aligner in enumerate(aligner_list):
    ...
    test_files = glob.glob("/hps/nobackup/goldman/charwest/omega_ai/data/test_tf_records/{0}/{0}.{1}.alignments.tfrecord".format(dataset_str, aligner))
    ...
    if not os.path.exists("/hps/nobackup/goldman/charwest/omega_ai/data/simulations/model_test_results/{0}/{1}".format(dataset_id, aligner)):
        os.makedirs("/hps/nobackup/goldman/charwest/omega_ai/data/simulations/model_test_results/{0}/{1}".format(dataset_id, aligner))
    roc_df.to_csv("/hps/nobackup/goldman/charwest/omega_ai/data/simulations/model_test_results/{0}/{1}/roc_res.csv".format(dataset_id, aligner), index=False)
    # Write ROC AUC to its own file
    with open("/hps/nobackup/goldman/charwest/omega_ai/data/simulations/model_test_results/{0}/{1}/roc_auc.txt".format(dataset_id, aligner), 'w') as f:
        f.write(str(roc_auc))
    ...
    pr_df.to_csv("/hps/nobackup/goldman/charwest/omega_ai/data/simulations/model_test_results/{0}/{1}/pr_res.csv".format(dataset_id, aligner), index=False)
    # Write PR AUC to its own file
    with open("/hps/nobackup/goldman/charwest/omega_ai/data/simulations/model_test_results/{0}/{1}/pr_auc.txt".format(dataset_id, aligner), 'w') as f:
        f.write(str(pr_auc))
