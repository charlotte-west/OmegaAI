...
for ((h=1; h<=$2; h++))
do
    sbatch --output=/hps/nobackup/goldman/charwest/omega_ai/sim_out/bm_divbase_indistNB_indrate01_tips8_posprop05_v3_%J.out \
                --error=/hps/nobackup/goldman/charwest/omega_ai/sim_out/bm_divbase_indistNB_indrate01_tips8_posprop05_v3_%J.err \
                --time=48:00:00 \
                --cpus-per-task=2 \
                --ntasks=1 \
                --mem=512M \
                --wrap "$conda_com && python test_to_groups.py $start $end 'baseline' 'NB' '0.1' 'bm_divbase_indistNB_indrate01_tips8_posprop05_v3' 'clustal' 8 0.5"
