...
def main():
    ...
    with open(file_list, "r") as f:
        print("printing f")
        print(f)
        test_files = [i.strip() for i in f.readlines()]
    print("fi_count")
    ...
    out_dir = base_dir + argv[1] + "/"  # argv[1] = dataset id
    ...
    # processes testing files
    for align_fi in test_files:
        print("got here")
        total_seen += 1
        # get y_label files
        aligner = align_fi.split("/")[-2].split("_")[0]
        label_fi = align_fi.replace(aligner + "_test_x", "test_y").replace("fas", "txt")
    ...
    # write the testign alignments into TFRecord files that will be used
    # for network testing
    with tf.io.TFRecordWriter(out_dir + aligner + "/" + "alignments_{}.tfrecord".format(fi_count)) as writer:
