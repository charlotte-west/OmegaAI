...
def seq_of_omega_as_str(RATES_file, w0, w1, w2):
    """
    Convert _RATES.txt file to a string of sequence of
    omega values of each codon site, separated by spaces.
    """
    with open(RATES_file, 'r') as file:
        for i, line in enumerate(file):
            if line.startswith('Site\t'):
                df = pd.read_csv(RATES_file, skiprows=range(0, i), sep="\t")
                return " ".join(pd.Series.map(df.Class, {0: str(w0), 1: str(w1), 2: str(w2)}))
    raise Exception("Wrong _RATES file format")
...
def simulate_align(dir, ID, aligner, shuffle=False):
    """
...
def simulate_true_align(dir, ID, shuffle=False):
    """
    Simulate an alignment using parameters defined in main.
    """
    tool_dir = dir + "tools/"
    ...
    print(f"Indelible place: {indelible}")
    # call INDELible
    indelible_start = time.time()
    os.system(indelible)
    indelible_end = time.time() - indelible_start
...
def main():
    # Universal Directory
    universal_dir = '/hps/nobackup/goldman/charwest/omega_ai/'
    # universal_dir = '/mnt/c/Users/15858/Desktop/test_dir/' # On my laptop
    ...
    if argv[7] == "test":
        basedir = universal_dir + "data/simulations/test_datasets/"
    else:
        basedir = universal_dir + "data/simulations/datasets/"
    ...
    os.chdir(folder)
    os.system("mkdir train_x train_y train_y_seq reference length")
    ...
    if argv[9] in ["32", "64", "128"]:
        with open(universal_dir + "data/gene_trees/artificial/{}_tips.tree".format(argv[9]), "r") as tree_f:
    ...
    for x in range(int(start),int(end)):
        ...
        if (len(argv) == 12):
            if (argv[11] == 'shuffle'):
                # print("got here 1")
                simulate_align(universal_dir, str(x), aligner, True)
            elif (argv[11] == 'true_align'):
                # print("got here 2")
                simulate_true_align(universal_dir, str(x), False)
        elif (len(argv) > 12):
            # print("got here 3")
            simulate_true_align(universal_dir, str(x), True)
        else:
            # print("got here 4")
            simulate_align(universal_dir, str(x), aligner)

        train_start = time.time()
        with open("train_y_seq/"+str(x)+".txt","w") as file:
            file.write(seq_of_omega_as_str("reference/site_classes/dna_RATES_"+str(x)+".txt", w0, w1, w2))
        train_end = time.time() - train_start
        print("Writing training data y_seq runtime: " + str(train_end), flush=True)
